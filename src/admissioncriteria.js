...
const AdmissionCriteria = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try{
      const res = await fetch("http://localhost:3004/api/pages/admission-criteria"); // or your correct endpoint
      const data = await res.json();
      console.log("Admission Criteria Sections:", data); // 🔍 See what comes back
      setSections(data); 
      }catch (err) {
        console.error("Failed to fetch admission criteria:", err);
      }
    };
